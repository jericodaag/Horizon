...
const AllUsers = () => {
  ...
  const { data: topCreators, isLoading: isLoadingCreators } = useGetTopCreators(8);
  ...
  const displayedUsers = useMemo(() => {
    ...
    if (activeTab === 'suggested') {
      ...
      if (topCreators && topCreators.length) {
        ...
        const topUsers = filteredUsers.filter(user => topCreatorIds.includes(user.$id));
        ...
        filteredUsers.sort((a, b) => (b.followerCount || 0) - (a.followerCount || 0));
  ...
  const renderLoading = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {Array(8).fill(0).map((_, index) => (
        <div key={index} className="bg-dark-2 rounded-xl p-5 flex flex-col gap-3 border border-dark-4">
      ...
      >
        ...
        <section className="mb-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-light-1">Discover People</h1>
              <p className="text-light-3 mt-1">Connect with creators and build your network</p>
            </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute top-2 right-3 flex gap-2">
          <div className="bg-dark-1/70 backdrop-blur-md px-2 py-1 rounded-full flex items-center gap-1">
            ...
            />
            <span className="text-xs font-medium text-light-2">{user.posts?.length || 0}</span>
          </div>
          ...
          <div className="bg-dark-1/70 backdrop-blur-md px-2 py-1 rounded-full flex items-center gap-1">
            <img
              ...
              className="w-3.5 h-3.5"
            />
            <span className="text-xs font-medium text-light-2">{user.followerCount || 0}</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 pt-0 relative">
        ...
        {/* User info */}
        <Link to={`/profile/${user.$id}`}>
          <h3 className="font-bold text-light-1 hover:text-primary-500 transition-colors">{user.name}</h3>
